/*
    ...
    VideoCore mailbox and property tags, for arch/m68k-emu68.

    Copied verbatim from arch/arm-native/soc/broadcom/2708/include/hardware,
    which is another target's tree. This describes SoC silicon, not a CPU, so
    it is identical for an m68k guest on the same board - but the port keeps
    its own copy rather than reaching across, the same way platform/bcm283x
    defines its register offsets locally instead of borrowing them.
*/
