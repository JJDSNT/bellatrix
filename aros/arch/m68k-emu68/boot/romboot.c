...
/*
    Adapted for arch/m68k-emu68: the ROMTag goes in .aros.romtag rather than
    ordinary .rodata. boot/emu68.ld explains why -- krnRomTagScanner() jumps to
    each tag's rt_EndSkip, and dosboot's covers roughly 70KB, so a tag placed
    anywhere after it in the image is silently skipped.

    rt_EndSkip points just past the tag itself for the same reason; the Amiga
    original points it at a romboot_end symbol at the end of its own module,
    which does not exist when the file is linked straight into the bootstrap.
*/
static const struct Resident rb_tag __attribute__((section(".aros.romtag"), used)) =
{
   RTC_MATCHWORD,
