...
/*
 * Aligned to 4 explicitly, because m68k does not do it implicitly.
 *
 * The fields add up to 162 bytes and the firmware's layout is 164: on ARM the
 * compiler rounds the struct up to a multiple of its widest member, so the two
 * trailing bytes appear on their own. m68k aligns to 2, so they do not, and
 * sizeof() came out 162 -- which the firmware rejects outright with
 * BCME_BUFTOOSHORT rather than misparsing.
 *
 * Every field before ea[] already sits at a multiple of 4, so no interior
 * offset moves; this only restores the tail padding the assert is checking.
 */
