...
static void sdhost_gpio_init(void)
{
    ...
    D(bug("[Emu68SD] %s: Muxing GPIO 48-53 to ALT0 (SDHOST)\n", __PRETTY_FUNCTION__));
...
static int FNAME_SDHOST(SDHostInit)(struct SDCardBase *SDCardBase)
{
    ...
    {
        D(bug("[Emu68SD] %s: not the card controller on this SoC\n", __PRETTY_FUNCTION__));
        if (MBoxMessage_)
    ...
    {
        bug("[Emu68SD] %s: Failed to open mbox.resource\n", __PRETTY_FUNCTION__);
        goto sdhost_fail;
    }

    if ((DMABase = OpenResource("dma.resource")) == NULL)
    {
        bug("[Emu68SD] %s: Failed to open dma.resource\n", __PRETTY_FUNCTION__);
        goto sdhost_fail;
    }
    ...
    {
        bug("[Emu68SD] %s: Failed to get core clock rate\n", __PRETTY_FUNCTION__);
        goto sdhost_fail;
    }

    D(bug("[Emu68SD] %s: Core clock rate: %u Hz\n", __PRETTY_FUNCTION__,
    ...
    if (bus && priv)
    {
         ...
         * Nothing to allocate.
         *
         * The other backend requires a DMA channel and a 64 KB bounce buffer
         * and refuses to initialise without them. This one moves data through
         * the FIFO with the CPU, which is what Emu68's own driver does on this
         * controller, so there is no channel to hold and no buffer to bounce
         * through -- and nothing to fail here.
         */
        bug("[Emu68SD] PIO backend, no DMA channel and no bounce buffer\n");
        ...
        if ((bus->sdcb_BusUnits = AllocPooled(SDCardBase->sdcard_MemPool, sizeof(struct sdcard_BusUnits))) != NULL)
        {
            ...
            D(bug("[Emu68SD] %s: Bus #%02u - 1 Unit(s) starting from %02u\n",
