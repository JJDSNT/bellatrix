    ...
    if (bus && priv)
    {
        ...
        /* Allocate DMA control block (must be 32-byte aligned) */
        {
            ...
            if (cb_raw)
            {
                ...
                /*
                 * Say it out loud, once per boot.
                 *
                 * Whether this driver uses DMA was answered twice from
                 * reading the source and got answered wrong the first time:
                 * dma.resource's own probe prints "allocated channel N" and
                 * then "released channel N", which is the resource testing
                 * itself, and it is easy to read as the card failing to keep a
                 * channel. The card's own channel never appeared in any log
                 * because this line was behind D().
                 *
                 * It costs one line and it makes the question unaskable.
                 */
                bug("[SDHost] DMA channel %d, bounce %p (%u KB)\n",
                    priv->dma_channel, priv->dma_bounce,
                    priv->dma_bounce_size / 1024);
            }
            else
            {
