...
#define HVS_PROBE_WINDOW_US 100000  /* 100 ms: 5-6 frames at 50-60 Hz */
...
/* The BCM system timer, 1 MHz, same source vcgfx_dma.c uses. Windows
 * measured in time rather than in loop iterations: the calibration reads
 * the HVS and the bit probes read the PixelValve, and a loop count that
 * means five frames through one of them meant one frame through the
 * other -- which is exactly how the first takeover on a Pi 3 measured a
 * one-frame window and gave up. */
static inline ULONG hvs_now_us(void)
{
    return AROS_LE2LONG(*(volatile ULONG *)SYSTIMER_CLO);
}

/* How many frames pass in a window of the given length? DISPSTATX's
 * scanline counter wraps once per frame, so counting wraps measures the
 * expected per-frame tick count directly, with no assumption about which
 * interrupt bit means what. */
static ULONG hvs_frames_in_window(ULONG window_us)
{
    ULONG last = hvs_rd(HVS_DISPSTATX(HVS_CHANNEL_HDMI))
               & HVS_DISPSTATX_LINE_MASK;
    ULONG start = hvs_now_us();
    ULONG frames = 0;

    while (hvs_now_us() - start < window_us)
    {
...
static void hvs_vsync_start(struct vc4_hvs_state *st)
{
    ...
    if (!st->hvs_VSyncMask)
    {
         ...
         */
        frames = hvs_frames_in_window(HVS_PROBE_WINDOW_US);
        bug("[VC4HVS] vsync: probe window = %u frames in %u us (vtot=%u)\n",
            frames, (ULONG)HVS_PROBE_WINDOW_US, vtot);

        if (frames < 2)
        {
            bug("[VC4HVS] vsync: no frames during the window, "
                "leaving masked\n");
            return;
        }
        ...
        for (bit = 9; bit >= 0; bit--)
        {
            ...
            i = hvs_now_us();
            while (hvs_now_us() - i < HVS_PROBE_WINDOW_US)
