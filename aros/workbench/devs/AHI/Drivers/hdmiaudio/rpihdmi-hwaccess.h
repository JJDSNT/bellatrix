 ...
 * what it can mean here.
 *
 * The driver this was copied from is built for AArch64 and emitted `dsb sy`
 * and `dmb sy` around every register access. The m68k assembler has never
 * heard of either, and under Emu68 there is no guest instruction that orders
 * the host's memory system anyway -- the JIT and the host CPU do that. What
 * is still needed is to stop the *compiler* reordering or caching an access
 * across the barrier, which a memory clobber does, and which is exactly what
 * arch/m68k-emu68/include/asm/cpu.h defines dsb() and dmb() to be for the
 * same reason in the DMA and USB paths.
 */
#include <asm/cpu.h>
