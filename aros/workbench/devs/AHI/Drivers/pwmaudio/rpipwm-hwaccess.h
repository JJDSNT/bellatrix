...
/*
 * See the same note in hdmiaudio: the driver this was copied from emits
 * `dsb sy` / `dmb sy`, which the m68k assembler does not know and which
 * would order nothing here anyway -- under Emu68 the JIT and the host CPU
 * own the memory system. What is still required is a compiler barrier, and
 * arch/m68k-emu68/include/asm/cpu.h defines dsb() and dmb() to be exactly
 * that for the DMA and USB paths.
 */
#include <asm/cpu.h>
