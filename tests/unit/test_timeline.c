...
int main(void)
{
    ...
    if (runtime_timeline_update(&t, 6000u, 77u) != 77u)
        return 1;

    runtime_timeline_set_mode(&t, RUNTIME_TIMELINE_REALTIME, 6000u, 77u);
    if (runtime_timeline_update(&t, 6500u, 0u) != 127u ||
        runtime_timeline_update(&t, 7000u, 0u) != 177u)
        return 2;

    runtime_timeline_set_mode(&t, RUNTIME_TIMELINE_HYBRID, 7000u, 177u);
    if (runtime_timeline_update(&t, 8000u, 180u) != 200u)
        return 3;
    if (runtime_timeline_update(&t, 9000u, 400u) != 377u)
        return 4;

    runtime_timeline_set_paused(&t, true, 9000u, 377u);
    if (runtime_timeline_update(&t, 19000u, 900u) != 377u)
        return 5;
    runtime_timeline_set_paused(&t, false, 19000u, 377u);
    if (runtime_timeline_update(&t, 19500u, 400u) != 420u)
        return 6;
    ...
    if (runtime_timeline_update(&t, 21000u, 900u) != 577u ||
        runtime_timeline_update(&t, 21200u, 900u) != 597u)
        return 7;
    ...
    if (runtime_timeline_update(&t, 22000u, 100u) != 597u)
        return 10;
    if (!t.clamp_active)
        return 11;
    if (runtime_timeline_update(&t, 30000u, 10000u) != 597u)
        return 12;
    if (t.clamp_active || t.realtime_target_cck != 597u)
        return 13;
    if (runtime_timeline_update(&t, 31000u, 10000u) != 697u)
        return 14;
