// src/machine/bus/gayle/gayle.h

#ifndef BELLATRIX_BUS_GAYLE_H
#define BELLATRIX_BUS_GAYLE_H

#include <stdint.h>
#include <stdbool.h>

#include "machine/bus/gayle/gayle_ide.h"

#define GAYLE_BASE_1200   0x00DA0000u
#define GAYLE_SIZE        0x10000u

typedef struct GayleState {
    GayleIde ide;

    /*
     * 4-bit GAYLE ID shift register.
     * ReadGayle() (KS LVO -136) reads $DE1000 four times; each read
     * shifts the register left and returns the old MSB in bit 7.
     * A1200 GAYLE ID = 0xD (binary 1101), yielding reads:
     *   0x80, 0x80, 0x00, 0x80  → assembled as 0xD by ReadGayle.
     */
    uint8_t id_shift;
} GayleState;

void gayle_init(
    GayleState *g,
    IsoImage *iso,
    uint8_t *atapi_buffer,
    uint32_t atapi_buffer_size
);

void gayle_reset(GayleState *g);

/* MMIO */
uint8_t  gayle_read8 (GayleState *g, uint32_t addr);
uint16_t gayle_read16(GayleState *g, uint32_t addr);
uint32_t gayle_read32(GayleState *g, uint32_t addr);

void gayle_write8 (GayleState *g, uint32_t addr, uint8_t  val);
void gayle_write16(GayleState *g, uint32_t addr, uint16_t val);
void gayle_write32(GayleState *g, uint32_t addr, uint32_t val);

/* GAYLE ID region: $DE0000-$DEFFFF */
#define GAYLE_ID_REGION_BASE  0x00DE0000u
#define GAYLE_ID_REGION_SIZE  0x00010000u
#define GAYLE_ID_REG          0x00DE1000u

/* Unified read/write — thin wrappers over the 8/16/32 variants above.
 * Used by machine.c dispatch and prepared for future bus.c integration. */
bool     gayle_owns_address(const GayleState *g, uint32_t addr);
uint32_t gayle_read (GayleState *g, uint32_t addr, unsigned size);
void     gayle_write(GayleState *g, uint32_t addr, uint32_t value, unsigned size);

#endif